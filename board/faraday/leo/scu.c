...
void setup_rtc(void)
{
	...
	writel(0, CONFIG_SCU_BASE + 0x200);
	writel(0 ,CONFIG_SCU_BASE + 0x204);
	writel(0, CONFIG_SCU_BASE + 0x210);

	//set rtc alarm
	writel(10, CONFIG_SCU_BASE + 0x208);	//alarm for 10 second

	//enable rtc alarm (bit 1)
	//rtc_alarm_enable_ctrl(1);
	writel(readl(CONFIG_SCU_BASE + 0x210) | 0x3, CONFIG_SCU_BASE + 0x210);
}
...
void scu_softoff_testing(u32 mode)
{
	...
	writel(0xFFFF0001, CONFIG_SCU_BASE + 0x00);

	//enable event
	writel(0x00F60005, CONFIG_SCU_BASE + 0x04);	//only rtc

	//write pwrup/ pwrdn bit
	data = readl(CONFIG_SCU_BASE + 0x08);
	data &= ~0x303;
	data |= 0x102;
	writel(data, CONFIG_SCU_BASE + 0x08);

	//clean PWRBTN_STS status
	writel(0x00000001, CONFIG_SCU_BASE + 0xC0);

	//enable RTC alarm event
	writel(0x0000EFFF, CONFIG_SCU_BASE + 0xC4);

	//program debounce time
	data = readl(CONFIG_SCU_BASE + 0x08);
	writel((data & 0xFFFFFFFF) | 0x0, CONFIG_SCU_BASE + 0x08);

	//set cpu idle checked by WFI
	writel(0x0000000E, CONFIG_SCU_BASE + 0xB4);
	src(CONFIG_SCU_BASE + 0xB4, 0xE, HSIZE_WORD);
	...
	data = readl(CONFIG_SCU_BASE + 0x04);
	writel(data & 0xfffffffb, CONFIG_SCU_BASE + 0x04);

	//setting CLK_EN for sleep mode !!
	writel(0x5a5a5a5a, CONFIG_SCU_BASE + 0x58);
	writel(0x5a5a5a5a, CONFIG_SCU_BASE + 0x68);
	writel(0x5a5a5a5a, CONFIG_SCU_BASE + 0x88);

	// enter sleep, disable DDR selfr_cmd
	writel(0x80000002, CONFIG_SCU_BASE + 0x20);
...
void scu_sleep_testing(u32 mode)
{
	...
	//enable event
	writel(readl(CONFIG_SCU_BASE + 0x04) | 0xfffc0000, CONFIG_SCU_BASE + 0x04);	//enable all button event

	//clean PWRBTN_STS status
	writel(0x00000001, CONFIG_SCU_BASE + 0xC0);

	//enable RTC alarm event
	writel(0x0000EFFF, CONFIG_SCU_BASE + 0xC4);	//wake up by which button
	...
	// enter sleep, disable DDR selfr_cmd
	writel(0x80000008, CONFIG_SCU_BASE + 0x20);
	...
	scu_print_00_events(readl(CONFIG_SCU_BASE + 0x00));
	scu_print_c0_events(readl(CONFIG_SCU_BASE + 0xc0));
}
