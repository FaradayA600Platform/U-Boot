...
#define TIMER_BASE 0x10100000 
#define TIMER3_BASE 0x10100000 
...
{
	writel(0, &regs->timer1_cr);
	writel(0, &regs->int_clr);

	writel(0x000000E3, &regs->timer1_cr); 
	writel(((sclk/(1*1000*1000))*usec), &regs->timer1_load);   

	while (!readl(&regs->raw_int))
		;
