...
#ifdef CONFIG_SOC_CA9
	*((volatile int *)(CONFIG_SCU_BASE + 0x200)) = 0x5878;//external irq source from intc
	*((volatile int *)(CONFIG_SCU_BASE + 0x238)) = 0x200; //enable external ahb
...
int do_irqinfo (cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	for (i = 0; i < NR_IRQS; i++) {
		if (IRQ_HANDLER[i].m_func != (interrupt_handler_t*) default_isr) {
			printf ("%02d  %08x  %08x %08d\n", i,
				(int)IRQ_HANDLER[i].m_func, (int)IRQ_HANDLER[i].m_data, (int)IRQ_HANDLER[i].count);
		}
	}
	puts ("\n");
	return (0);
