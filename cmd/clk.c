...
int __weak soc_clk_set(u32 mode)
{
	puts("Not implemented\n");
	return 1;
}

static int do_clk_set(struct cmd_tbl *cmdtp, int flag, int argc,
		       char *const argv[])
{
	u32 mode;
...
static struct cmd_tbl cmd_clk_sub[] = {
	U_BOOT_CMD_MKENT(set, 2, 1, do_clk_set, "", ""),
...
static char clk_help_text[] =
	"set - Set clock frequencies\n"
	"dump - Print clock frequencies";
#endif

U_BOOT_CMD(clk, 3, 1, do_clk, "CLK sub-system", clk_help_text);
