...
#if defined(CONFIG_UMS_RAM)
//#if defined(CONFIG_ARCH_A380)||defined(CONFIG_MACH_HGU)
#include <asm/io.h>
...
#if  defined(CONFIG_UMS_RAM)
//#if defined(CONFIG_ARCH_A380)||defined(CONFIG_MACH_HGU)			
	sprintf((char *) (buf + 8), "%-8s%-16s%04x","U-boot   ","Storage Gadget",1);
#else				
...
#if  defined(CONFIG_UMS_RAM)
// defined(CONFIG_ARCH_A380) ||defined(CONFIG_MACH_HGU)
#define USBDEV_DISK_SIZE		(2*1024*1024)
#define USBDEV_DISK_SIZE_W1	((((USBDEV_DISK_SIZE/512))&0xFF000000)>>24)
#define USBDEV_DISK_SIZE_W2	((((USBDEV_DISK_SIZE/512))&0x00FF0000)>>16)
#define USBDEV_DISK_SIZE_W3	((((USBDEV_DISK_SIZE/512))&0x0000FF00)>>8)
#define USBDEV_DISK_SIZE_W4	((((USBDEV_DISK_SIZE/512))&0x000000FF)>>0)
...
#if  defined(CONFIG_UMS_RAM)	
//#ifdef UMS_RAM    //if defined(CONFIG_ARCH_A380)||defined(CONFIG_MACH_HGU)
	buf[0] = USBDEV_DISK_SIZE_W1;	
	buf[1] = USBDEV_DISK_SIZE_W2;	
//	buf[2] = 0x20;	
	buf[2] = USBDEV_DISK_SIZE_W3;
	buf[3] = USBDEV_DISK_SIZE_W4;	
#else
			...
			struct fsg_buffhd *bh)
{
	...
	u8		*buf = (u8 *) bh->buf;
#if  defined(CONFIG_UMS_RAM)
//#ifdef UMS_RAM  //defined(CONFIG_ARCH_A380)||defined(CONFIG_MACH_HGU)
	buf[0] 	= 0;
	buf[1] 	= 0;
	buf[2] 	= 0;
...
#if defined(CONFIG_UMS_RAM)	
//defined(CONFIG_ARCH_A380) ||defined(CONFIG_MACH_HGU)
	if(common->cmnd[0]==SC_READ_10)
	{		
		tmpVal = (common->cmnd[2] << 24) |(common->cmnd[3] << 16) |(common->cmnd[4] << 8 ) |(common->cmnd[5] << 0 ) ;
		if(diskTmpBuf == NULL) 
			diskTmpBuf = (u8 *)malloc(USBDEV_DISK_SIZE);

		pSCSIRespData = diskTmpBuf+(tmpVal<<9);		
		fotg210_dma_scsi_xfer(common->fsg->bulk_in, common->data_size, DMACTRL_MEM2FIFO, FIFO_IN);
		bh->state = BUF_STATE_EMPTY;

		return 0;
	}
	if(common->cmnd[0]==SC_WRITE_10)
	{
		tmpVal = (common->cmnd[2] << 24) |(common->cmnd[3] << 16) |(common->cmnd[4] << 8 ) |(common->cmnd[5] << 0 ) ;
		if(diskTmpBuf == NULL) 
			diskTmpBuf = (u8 *)malloc(USBDEV_DISK_SIZE);
		...
		pSCSIRespData = diskTmpBuf+(tmpVal<<9);
		fotg210_dma_scsi_xfer(common->fsg->bulk_out, common->data_size, DMACTRL_FIFO2MEM, FIFO_OUT);			
		bh->state = BUF_STATE_EMPTY;
		return 0;
	}
#endif
...
#if  1	//defined(CONFIG_UMS_RAM)
// defined(CONFIG_ARCH_A380)||defined(CONFIG_MACH_HGU)
//			printf("START_TRANSFER_OR3\n");
			START_TRANSFER_OR(common, bulk_in, bh->inreq,
					  &bh->inreq_busy, &bh->state)
				/* Don't know what to do if
				 * common->fsg is NULL */
