...
#define CFG_EPX_MAX_PACKET_SIZE	64

#define CFG_CMD_TIMEOUT (CONFIG_SYS_HZ >> 2) /* 250 ms */

#define CONFIG_FOTG210_BASE FOTG210_BASE0
...
static int fotg210_reset(struct fotg210_chip *chip)
{
	...
	writel(0x3F, &regs->gisr0);
	writel(0xFFFFF, &regs->gisr1);
	writel(0x7FF, &regs->gisr2);
#else	
	writel(0, &regs->gisr0);
	writel(0, &regs->gisr1);
	writel(0, &regs->gisr2);
	...
	writel(readl(&regs->phy_tmsr)&(~PHYTMSR_UNPLUG), &regs->gisr);
...
#if defined(CONFIG_ARCH_A380)
	writel(IMR_IRQLL | IMR_HOST | IMR_OTG, &regs->imr);
#else	
...
extern u8 *pSCSIRespData;
#define PAYLOAD_SIZE 64
void fotg210_dma_scsi_xfer(struct fotg210_ep *ep,u32 size,u8 dma_type,u8 fifo)
{
	struct fotg210_chip *chip = ep->chip;
	struct fotg210_regs *regs = chip->regs;
	int i,val;;
	
		for(i=0;i<size/PAYLOAD_SIZE;i++)
		{
			writel(PAYLOAD_SIZE<<DMACTRL_LEN_SHIFT|dma_type, &regs->dma_ctrl);
			writel(DMAFIFO_FIFO(fifo), &regs->dma_fifo);
			writel(pSCSIRespData+i*PAYLOAD_SIZE, &regs->dma_addr);					
			writel(PAYLOAD_SIZE<<DMACTRL_LEN_SHIFT|dma_type|DMACTRL_START, &regs->dma_ctrl);
			while(1)
			{
				val = readl(&regs->gisr2);
				if(val & GIMR2_DMAFIN)
				{
					writel(val,&regs->gisr2);	
					break;
				}
				if(val & GIMR2_DMAERR)
					break;				
...
static int fotg210_dma(struct fotg210_ep *ep, struct fotg210_request *req)
{
	...
	/* 3. DMA target setup */
	dummy_len = ((len/CONFIG_SYS_CACHELINE_SIZE)+1)*CONFIG_SYS_CACHELINE_SIZE;	//maybe flush one more cache line,doesn't matter	
	if (ep->desc->bEndpointAddress & USB_DIR_IN)
		flush_dcache_range((ulong)buf, (ulong)buf + dummy_len);
	else
		invalidate_dcache_range((ulong)buf, (ulong)buf + dummy_len);
...
static void fotg210_recv(struct fotg210_chip *chip, int ep_id)
{
	...
	if (list_empty(&chip->gadget.ep_list)) {
...
static int fotg210_start(struct usb_gadget *_gadget,
		struct usb_gadget_driver *driver)
{
...
static int fotg210_stop(struct usb_gadget *_gadget)
{
	struct fotg210_chip *chip = container_of(_gadget, struct fotg210_chip, gadget);
	chip->driver = NULL;
	return 0;
...
static struct usb_gadget_ops fotg210_gadget_ops = {
	...
	.udc_start = 	fotg210_start,
	.udc_stop = 	fotg210_stop,		
};
...
void fotg210_setup_ep()
{
 ...
 int fotg210_gadget_probe(struct udevice *dev)
{
	struct fotg210_chip *chip  = &controller;
	...
	chip->dev = dev;
	chip->regs = controller.regs;	
	chip->gadget.name = controller.gadget.name;	
	chip->gadget.ops = controller.gadget.ops;	
	chip->gadget.ep0 = &(controller.ep[0].ep);
	chip->gadget.speed = controller.gadget.speed;
	chip->gadget.is_dualspeed = controller.gadget.is_dualspeed;
	chip->gadget.is_otg = controller.gadget.is_otg,
	chip->gadget.is_a_peripheral = controller.gadget.is_a_peripheral ,
	chip->gadget.b_hnp_enable = controller.gadget.b_hnp_enable ,
	chip->gadget.a_hnp_support = controller.gadget.a_hnp_support,
	chip->gadget.a_alt_hnp_support = controller.gadget.a_alt_hnp_support,
	...
	usb_add_gadget_udc((struct device *)dev, &chip->gadget);
	return 0;
}

static int fotg210_gadget_remove(struct udevice *dev)
{
	struct fotg210_chip *chip = dev_get_priv(dev);
	usb_del_gadget_udc(&chip->gadget);
	return 0;
}

static const struct udevice_id fotg210_udc_ids[] = {
	{ .compatible = "fotg210,fotg210_udc" },
	{ }
};
U_BOOT_DRIVER(fotg210_udc) = {
	.name	= "fotg210_udc",
	.id	= UCLASS_USB_GADGET_GENERIC,
	.of_match = fotg210_udc_ids,
	.probe = fotg210_gadget_probe,
	.remove = fotg210_gadget_remove,
	.priv_auto	= sizeof(struct fotg210_chip),
