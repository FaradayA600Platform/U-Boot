...
#define MIIM_88E1xxx_PHY_DOWNSHIFT_EN		0x0800
#define MIIM_88E1xxx_PHY_DOWNSHIFT_SHIFT	12
#define MIIM_88E1xxx_PHY_DOWNSHIFT_MASK	0x7800
...
/* Marvell 88E1116R */
static int m88e1116r_config(struct phy_device *phydev)
{
	u16 reg;
	int mscr;
	...
	reg |= MIIM_88E1xxx_PHY_MDI_X_AUTO;
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1xxx_PHY_SCR, reg);

	/* Set Downshift */
	reg = phy_read(phydev, MDIO_DEVAD_NONE, MIIM_88E1xxx_PHY_SCR);
	reg &= MIIM_88E1xxx_PHY_DOWNSHIFT_MASK;
	reg |= ((8 - 1) << MIIM_88E1xxx_PHY_DOWNSHIFT_SHIFT);
	reg |= MIIM_88E1xxx_PHY_DOWNSHIFT_EN;
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1xxx_PHY_SCR, reg);
	...
	if (phy_interface_is_rgmii(phydev)) {
		...
		mscr = phy_read(phydev, MDIO_DEVAD_NONE, MIIM_88E151x_PHY_MSCR);
		mscr &= ~(MIIM_88E151x_RGMII_RXTX_DELAY);
		if ((phydev->interface == PHY_INTERFACE_MODE_RGMII) ||
		    (phydev->interface == PHY_INTERFACE_MODE_RGMII_ID))
			mscr |= (MIIM_88E151x_RGMII_RX_DELAY | MIIM_88E151x_RGMII_TX_DELAY);
		else if (phydev->interface == PHY_INTERFACE_MODE_RGMII_RXID)
			mscr |= MIIM_88E151x_RGMII_RX_DELAY;
		else if (phydev->interface == PHY_INTERFACE_MODE_RGMII_TXID)
			mscr |= MIIM_88E151x_RGMII_TX_DELAY;
		phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E151x_PHY_MSCR, mscr);
	}
