...
struct ftspi020_spi_platdata {
	struct ftspi020_spi_regs *regs;
};
...
static void spi_cs_activate(struct udevice *dev, uint cs)
{
	...
	struct ftspi020_spi_regs *const regs = priv->regs;
...
static int ftspi020_spi_claim_bus(struct udevice *dev)
{
	struct udevice *bus = dev->parent;
	struct ftspi020_spi_priv *priv = dev_get_priv(bus);
	struct ftspi020_spi_regs *const regs = priv->regs;
...
static int ftspi020_spi_release_bus(struct udevice *dev)
{
	struct udevice *bus = dev->parent;
	struct ftspi020_spi_priv *priv = dev_get_priv(bus);
	struct ftspi020_spi_regs *const regs = priv->regs;
...
{
	struct ftspi020_spi_platdata *plat = dev_get_platdata(bus);
	struct ftspi020_spi_priv *priv = dev_get_priv(bus);
...
static int ftspi020_spi_ofdata_to_platdata(struct udevice *bus)
{
	...
	plat->regs = map_physmem(dev_get_addr(bus),
				 sizeof(struct ftspi020_spi_regs),
				 MAP_NOCACHE);
...
static const struct dm_spi_ops ftspi020_spi_ops = {
	.claim_bus	= ftspi020_spi_claim_bus,
	.release_bus	= ftspi020_spi_release_bus,
...
static const struct udevice_id ftspi020_spi_ids[] = {
	{ .compatible = "faraday,spi-1.0" },
	{}
};

U_BOOT_DRIVER(ftspi020_spi) = {
	.name	= "ftspi020_spi",
	.id	= UCLASS_SPI,
	.of_match = ftspi020_spi_ids,
	.ops	= &ftspi020_spi_ops,
	.ofdata_to_platdata = ftspi020_spi_ofdata_to_platdata,
	.platdata_auto_alloc_size = sizeof(struct ftspi020_spi_platdata),
	.priv_auto_alloc_size = sizeof(struct ftspi020_spi_priv),
